...
time1 = (float(indgen(365*44))/365.0) + 1950.0
time2 = indgen(365)
time3 = indgen(44)+1950

; loop to plot each of the 23 variables
for i=0,22 do begin
	
	; first plot the daily data for all 44 years
	plot,time1,d(i,*,*),/yno,title=names[i],xr=[1950,1994],xs=1
	; wait for a key press
	junk=get_kbrd(1)
	 
	; next make an average year and plot that
	avgyr = reform(total(d(i,*,*),3))/44.0
	...
	plot,time3,annavg,/yno,title=names[i]+' : Yearly Average',xr=[1950,1993],$
		xs=1
	junk=get_kbrd(1)
endfor
return
