...
phenarray_struct* phen)
{
	...
	{
		printf("Error allocating for phen->curgrowth, prephenology()\n");
		ok=0;
	}
	if (ok && !(phen->remdays_transfer = (int*) malloc(ndays*sizeof(int))))
	{
		printf("Error allocating for phen->remdays_transfer, prephenology()\n");
		ok=0;
	}
	if (ok && !(phen->remdays_litfall = (int*) malloc(ndays*sizeof(int))))
	{
		printf("Error allocating for phen->remdays_litfall, prephenology()\n");
		ok=0;
	}
	if (ok && !(phen->predays_transfer = (int*) malloc(ndays*sizeof(int))))
	{
		printf("Error allocating for phen->predays_transfer, prephenology()\n");
		ok=0;
	}
	if (ok && !(phen->predays_litfall = (int*) malloc(ndays*sizeof(int))))
	{
		printf("Error allocating for phen->predays_litfall, prephenology()\n");
		ok=0;
	}
