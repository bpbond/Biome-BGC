...
int tipping(const siteconst_struct* sitec, const epconst_struct* epc, epvar_struct* epv, wstate_struct* ws, wflux_struct* wf)
{
	...
	int ll=0;
	double INFILT;
	double drain_coeff; //ratio of the drained of water of a given day
	...
	if (INFILT > 0)
	{
		...
		for  (layer=0 ; layer<N_SOILLAYERS-1; layer++) 
		{
			...
			{
				conduct_sat = sitec->hydr_conduct_sat[layer] * m_to_cm * NSEC_IN_DAY;
			}
			else
			{
				conduct_sat = 0;
			...
			/* hydraulic conductivity in actual layer (cm/day = m/s * 100 * NSEC_IN_DAY) 
