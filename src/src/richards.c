...
int richards(const siteconst_struct* sitec, const epconst_struct* epc, epvar_struct* epv, wstate_struct* ws, wflux_struct* wf)
{
	...
	double soilw0, soilw1, vwc0, vwc1, vwc_diff0, vwc_diff1, diff0, diff1, limit, soilw_SAT_total, soilw_total;
	...
	for (layer=0 ; layer < N_SOILLAYERS ; layer++)
	{
		...
		soilw_SAT_total += sitec->vwc_sat[layer]  * sitec->soillayer_thickness[layer] * water_density;
		soilw_total     += ws->soilw[layer];
	}
	...
	if (soilw_total + INFILT > soilw_SAT_total)
	{
		wf->soilw_to_pondw = soilw_total + INFILT - soilw_SAT_total;
	}
	...
	if (ws->pond_water > sitec->pondmax)
	{
		wf->prcp_to_runoff += (ws->pond_water - sitec->pondmax);
		ws->pond_water = sitec->pondmax;
	}
